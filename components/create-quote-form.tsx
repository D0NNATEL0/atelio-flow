...
import { addStoredQuote, formatEuroAmount, getStoredClients } from "@/lib/browser-storage";
...
export function CreateQuoteForm() {
  ...
  const [quoteTitle, setQuoteTitle] = useState("Prestation créative");
  ...
  const validityLabel = hasValidity ? `Valable ${validityDays} jours` : "Sans durée imposée";
  ...
  function goNext() {
    setStep((currentStep) => (currentStep < 3 ? ((currentStep + 1) as QuoteStep) : currentStep));
  }

  function goBack() {
    setStep((currentStep) => (currentStep > 1 ? ((currentStep - 1) as QuoteStep) : currentStep));
  }

  function persistQuote(status: "Brouillon" | "Envoyé") {
    addStoredQuote({
      id: `DV-${new Date().getFullYear()}-${String(Date.now()).slice(-3)}`,
    ...
    startTransition(async () => {
      ...
      const printableHtml = `
        ...
        <html lang="fr">
          <head>
            <meta charset="utf-8" />
            <title>Devis ${escapeHtml(clientName || "Atelio")}</title>
            <style>
              :root {
            ...
            <main class="sheet">
              <section class="head">
                ...
                <div class="meta">
                  ...
                  <h2>DV-${new Date().getFullYear()}-Aperçu</h2>
                  <p class="muted">${escapeHtml(validityLabel)}</p>
                </div>
              </section>
              ...
              <section class="card">
                ...
                <h3 style="margin: 10px 0 8px; font-size: 26px;">${escapeHtml(quoteTitle)}</h3>
                <p class="muted">Aperçu de la proposition avant émission.</p>
              </section>
      ...
      <div className="quote-builder-steps">
        {[
          ...
          <button
            ...
            key={item.id}
            onClick={() => setStep(item.id as QuoteStep)}
            type="button"
          >
            {item.label}
          </button>
        ...
        <article className="account-card quote-builder-panel">
          {step === 1 ? (
            <>
              ...
              <p className="metric-subtext">
                Ces données seront imprimées sur le devis. Le logo du compte est repris automatiquement si disponible.
              </p>
              ...
              <div className="account-form-grid quote-form-grid">
                ...
                <label className="field">
                  <span>Email pro</span>
                  <input className="input" onChange={(e) => setProfessionalEmail(e.target.value)} type="email" value={professionalEmail} />
                </label>
          ...
          {step === 2 ? (
            <>
              ...
              <p className="metric-subtext">
                Tu peux repartir d’un client déjà créé, puis ajuster les informations du devis.
              </p>
              ...
              <div className="account-form-grid quote-form-grid">
                ...
                <label className="field field-span-2">
                  <span>Intitulé du devis</span>
                  <input className="input" onChange={(e) => setQuoteTitle(e.target.value)} value={quoteTitle} />
                </label>
                ...
                <label className="field field-span-2">
                  ...
                  <div className="quote-validity-row">
                    <label className="quote-choice-card">
                      <input
                        checked={!hasValidity}
                        name="validity-mode"
                        onChange={() => setHasValidity(false)}
                        type="radio"
                      />
                      <span>Pas besoin</span>
                    </label>
                    ...
                    <label className="quote-choice-card">
                      <input
                        checked={hasValidity}
                        name="validity-mode"
                        onChange={() => setHasValidity(true)}
                        type="radio"
                      />
                      <span>Choisir un nombre de jours</span>
                    </label>
                  </div>
                </label>
          ...
          {step === 3 ? (
            <>
              ...
              <p className="metric-subtext">
                Ajuste le style du document avant de l’ajouter à ton suivi.
              </p>
          ...
          <div className="quote-builder-actions">
            ...
            {step < 3 ? (
              ...
              <button className="button button-secondary" disabled type="button">
                Choisis une action à droite
              </button>
            )}
          </div>
        </article>

        <aside className="account-card quote-preview-panel">
          <span className="panel-label">Aperçu du document</span>
          <div
          ...
          >
            <div className="quote-preview-head">
              <div className="quote-preview-brand">
                <div className="quote-preview-logo">
                  {logoUrl ? <img alt="Logo du devis" src={logoUrl} /> : <span>{previewInitial}</span>}
                </div>
              ...
              <div className="quote-preview-meta">
                <strong>DEVIS</strong>
                <span>{`DV-${new Date().getFullYear()}-Aperçu`}</span>
            ...
            <div className="quote-preview-title-block">
              <span className="quote-preview-label">Objet</span>
              <h4>{quoteTitle}</h4>
              <p>Un aperçu simple de la proposition avant émission.</p>
            </div>
          ...
          {step === 3 ? (
            <div className="quote-preview-final-actions">
              <button className="button button-secondary" disabled={isPending} onClick={handleDownload} type="button">
                {isPending ? "Préparation..." : "Télécharger le devis"}
              </button>
